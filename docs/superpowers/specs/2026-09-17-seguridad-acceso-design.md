...
- Caja y admin: `expira_en` se recalcula en cada petición **que inicia la persona** (30 y 15 minutos). Mesero: sin expiración por inactividad.
- **Las peticiones automáticas no cuentan como uso** (decisión del 2026-09-18, para cumplir lo aprobado: la pantalla "se cierra sola si nadie la toca"). Las recargas periódicas y las que dispara un aviso en vivo llevan la cabecera `X-Automatica: 1`; el guardia las valida igual que cualquier otra (dispositivo, sesión viva y rol), pero **no** renueva `expira_en`. Así, una pantalla de admin o de caja abierta sin que nadie la toque vence en su plazo aunque siga recargando datos, y la siguiente recarga automática recibe 401 y pone el teclado de PIN encima. No hay riesgo en confiar en la cabecera: omitirla solo mantiene el comportamiento anterior (renovar), y ponerla solo acorta la vida de la propia sesión.
